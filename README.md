...
資産総額の推移をブラウザ上で確認できます。データ処理はローカルのブラウザ内で完結し、外部サーバーへ資産情報を送信しません。
...
- 資産総額の推移を折れ線グラフで表示
- クイック期間、対象年、日付抽出、月末抽出に対応
- クイック、対象年の月次、年月指定の日次を切り替え
- 月次の前年比較、日次の前年同月比較、直近1年の前年同期間比較
- 移動平均の表示
...
3. 「クイック」「月次」「日次」から表示方法を選び、配下の期間条件を指定します（初期表示は「クイック」の「1年」です）。
4. 「再取得・描画」で最新データを取得してグラフを更新します。
5. 必要に応じてCSV保存、CSVコピー、画像コピーを実行します。

グラフ上をドラッグすると一時的に拡大表示できます。ズーム解除で元の期間表示に戻り、CSV出力には影響しません。

月次、日次、または「1年」表示では、画面下部の「前年比較」を有効にすると、対象年と前年、選択した年月と前年同月、または直近1年と前年同期間の資産推移を同じグラフ上で比較できます。月次では選択年と前年の各12か月、1年表示では比較境界月を含む直近2年分だけを取得します。

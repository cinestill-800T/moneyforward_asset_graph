...
- **キーワード選択**: 「Amazon」などのキーワードで対象を絞り込んで一括選択できます。
- **未分類の自動選択**: 「未分類」の項目だけをワンクリックで選択します（振替などは自動的に除外されます）。
- **カテゴリ一括適用**: 大項目・中項目を指定して、選択したすべての行に一括適用します。
- **安全な更新**: ドロップダウン操作をエミュレートし、確実に変更を保存します。
- **パネル移動 (New v1.3.2)**: パネルのヘッダーをドラッグすることで、画面上の好きな位置に移動できます。
...
3. **キーワード選択**: 入力欄に文字を入れて「選択」を押すと、該当する行だけがチェックされます。
4. **手動選択**: 表の左端に追加されたチェックボックスで対象を選択します。
5. パネルで大項目・中項目を選び、「選択した項目に適用」をクリックします。
6. 自動的にドロップダウンが操作され、カテゴリが更新されます（処理中はページを閉じないでください）。
